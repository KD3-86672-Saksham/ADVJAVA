...
public class CandidateDaoImpl extends Dao implements CandidateDao {
	...
	private PreparedStatement stmtCastVote;
	
	public CandidateDaoImpl() throws Exception {
		String sqlCaseVote = "Update candidates Set votes = votes + 1 where id = ?";
		stmtCastVote = con.prepareStatement(sqlCaseVote);
	...
	public int castVote(int id) throws Exception {
		stmtCastVote.setInt(1, id);
		int count = stmtCastVote.executeUpdate();	
		return count;
